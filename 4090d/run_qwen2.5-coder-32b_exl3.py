#!/usr/bin/env python3
"""
Qwen2.5-Coder-32B EXL2 - 基于 exllamav3 (支持投机解码)

使用方法:
  1. 启动: nohup python3 run_qwen2.5-coder-32b_exl3.py > /tmp/model_32b.log 2>&1 &
  2. 测试: curl http://localhost:11434/v1/models
  3. 关闭: pkill -f run_qwen2.5-coder-32b_exl3.py
"""
...
print(f"Loading main model: {MAIN_MODEL_DIR}...")

config = Config.from_directory(MAIN_MODEL_DIR)
...
draft_cache = Cache(draft_model, max_num_tokens=MAX_SEQ_LEN, layer_type=CacheLayer_fp16)
draft_model.load(use_per_device=None, progressbar=True)

print("Creating generator with speculative decoding...")
generator = Generator(
    model=model,
    cache=cache,
    tokenizer=tokenizer,
...
print("")
print("=" * 60)
print("🚀 Qwen2.5-Coder-32B 服务已启动! (exllamav3)")
print("=" * 60)
print(f"模型: 32B EXL2 (exllamav3) + 0.5B 草稿模型")
...
async def list_models():
    return {
        ...
        "data": [
            {
                "id": "qwen2.5-coder-32b-exl3",
                "object": "model",
                "created": int(time.time()),
                "owned_by": "qwen",
                "permission": [],
                "root": "qwen2.5-coder-32b-exl3",
            }
        ],
    }
...
def generate_text(prompt, max_new_tokens=1024, temperature=0.0):
    """Generate text using exllamav3 Generator with speculative decoding"""
...
async def generate_completion(data):
    ...
    if tool_calls:
        return {
            ...
            "created": int(time.time()),
            "model": data.get("model", "qwen2.5-coder-32b-exl3"),
            "choices": [{
                "index": 0,
...
async def chat_completions(request: Request):
    ...
    messages = data.get("messages", [])
    tools = data.get("tools", [])
    model_name = data.get("model", "qwen2.5-coder-32b-exl3")
    max_tokens = data.get("max_tokens", MAX_SEQ_LEN - 1024)
