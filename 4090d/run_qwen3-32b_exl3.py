...
使用方法:
  1. 启动: nohup python3 run_qwen3-32b_exl3.py > /tmp/model_32b.log 2>&1 &
  2. 测试: curl http://localhost:11434/v1/models
  3. 关闭: pkill -f run_qwen3-32b_exl3.py
"""
...
MAIN_MODEL_DIR = "/opt/gguf/Qwen3-32B-exl3"
MAX_SEQ_LEN = 32768
PORT = 11434

print(f"Loading Qwen3-32B model from {MAIN_MODEL_DIR}...")
...
async def generate_completion(data):
    ...
    if tool_calls:
        return {
            ...
            "created": int(time.time()),
            "model": data.get("model", "qwen3-32b-exl3"),
            "choices": [{
                "index": 0,
...
async def chat_completions(request: Request):
    ...
    messages = data.get("messages", [])
    tools = data.get("tools", [])
    model_name = data.get("model", "qwen3-32b-exl3")
    max_tokens = data.get("max_tokens", MAX_SEQ_LEN - 1024)
