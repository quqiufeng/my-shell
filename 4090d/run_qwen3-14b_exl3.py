#!/usr/bin/env python3
"""
Qwen3-14B EXL3 启动脚本 - RTX 4090D 24GB
...
[必需]
  --model-dir         模型目录路径 (默认: /opt/gguf/Qwen3-14B-exl3)

[可选]
  --port              API 服务端口 (默认: 11434)
  --host              监听地址 (默认: 0.0.0.0)
  --cache-tokens      KV Cache token 数 (默认: 24576)
...
# 启动服务
nohup python3 /home/dministrator/my-shell/4090d/run_qwen3-14b_exl3.py > /tmp/qwen3_14b_exl3_$(date +%Y%m%d_%H%M%S).log 2>&1 & echo $!

# 查看日志
tail -f /tmp/qwen3_14b_exl3_*.log

# 查看进程
ps aux | grep run_qwen3-14b_exl3

# 停止服务
pkill -f run_qwen3-14b_exl3
...
def main():
    ...
    async def responses_endpoint(request: Request):
        ...
        return await handle_response(request, handler, "qwen3-14b-exl3")
