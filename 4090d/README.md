...
## 🔬 模型蒸馏 (Qwen2.5-Coder-32B + Claude 推理)

基于 Qwen2.5-Coder-32B 蒸馏 Claude 推理能力，获得代码+推理双强模型。

### 蒸馏原理

#### 什么是知识蒸馏
知识蒸馏 (Knowledge Distillation) 是将大模型(Teacher)的知识迁移到小模型(Student)的技术：

```
Teacher (Claude 4.6 Opus) → 提供推理轨迹
         ↓
Student (Qwen2.5-Coder-32B) → 学习推理模式
         ↓
最终模型 → 代码能力强 + 推理思维链
```
...
| 类型 | 说明 | 显存需求 |
|------|------|----------|
| 软目标蒸馏 | 学习 Teacher 的输出概率分布 | 高 |
| 硬目标蒸馏 | 学习 Teacher 的输出答案 | 中 |
| 思维链蒸馏 | 学习 Teacher 的 `<think>` 推理过程 | 中 |

#### 本项目采用方法
- **SFT + LoRA** - 仅训练 response 部分
- **train_on_responses_only** - 仅在 `<think>` 和答案上计算 loss
- **Mask 策略** - 忽略 `<|im_start|>assistant\n<think>` 部分

### 数据集

| 数据集 | 大小 | 说明 |
|--------|------|------|
| `nohurry/Opus-4.6-Reasoning-3000x-filtered` | 7.2MB | Claude 4.6 推理轨迹 (已下载) |
| `Roman1111111/claude-opus-4.6-10000x` | 13MB | 大规模蒸馏数据 (已下载) |
| `TeichAI/claude-4.5-opus-high-reasoning-250x` | 8.6MB | 高强度结构化推理 (已下载) |
| `Jackrong/Qwen3.5-reasoning-700x` | 57MB | 额外推理样本 (已下载) |
...
| 1 | 下载蒸馏数据集 | ✅ 已完成 |
| 2 | 安装 Unsloth 环境 | ⏳ 待执行 |
| 3 | 转换模型格式 (exl2 → GGUF) | ⏳ 待执行 |
| 4 | 准备 QLoRA 训练脚本 | ⏳ 待执行 |
| 5 | 执行 QLoRA 训练 (10-20小时) | ⏳ 待执行 |
| 6 | 合并 LoRA 权重 | ⏳ 待执行 |
| 7 | 转换回 EXL2 量化 | ⏳ 待执行 |
| 8 | 测试性能 | ⏳ 待执行 |
...
#### 步骤 1: 下载数据集 ✅
```bash
# 数据集已下载到 /opt/datasets/
ls -lh /opt/datasets/*/
```

#### 步骤 2: 安装 Unsloth
```bash
# 安装 Unsloth (4090D 需要 CUDA 12+)
...
```bash
# exl2 → GGUF (需要转换)
python3 /opt/llama.cpp/convert_llama.py \
  --outfile /opt/models/Qwen2.5-Coder-32B-GGUF/ \
  --outtype f16 \
  /opt/gguf/exl2_4_0/
```
...
def load_dataset():
    datasets = [
        "/opt/datasets/nohurry_Opus-4.6-Reasoning-3000x-filtered/distilled_corpus_400k_with_cot-filtered.jsonl",
        "/opt/datasets/Roman1111111_claude-opus-4.6-10000x/Roman1111111/opus46_final.jsonl",
    ]
    # 合并处理...
    return dataset
...
```bash
# 开始 QLoRA 训练
python3 distill_coder_32b.py

# 预计时间: 10-20 小时 (取决于 batch_size)
# 显存需求: ~16-20GB (4090D 可行)
```
...
merged_model.save_pretrained("/opt/models/Qwen2.5-Coder-32B-Claude-Distilled/")
```

#### 步骤 7: 转换 EXL2
```bash
# 使用 llama.cpp 转换为 EXL2
python3 /opt/exllamav2/convert.py \
  --model /opt/models/Qwen2.5-Coder-32B-Claude-Distilled/ \
  --output /opt/gguf/Qwen2.5-Coder-32B-Claude-4.6-Opus-exl2/ \
  --bits 4
```
...
data = {
    'messages': [{'role': 'user', 'content': '用Python实现一个红黑树'}],
    'max_tokens': 1000
}
...
- [Unsloth AI](https://unsloth.ai/) - 高效微调框架
- [llama.cpp](https://github.com/ggerganov/llama.cpp) - 模型转换
- [ExLlamaV2](https://github.com/turboderp/exllamav2) - 推理加速
```
