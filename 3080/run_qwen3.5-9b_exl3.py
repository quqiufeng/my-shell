...
def format_prompt(messages):
    ...
    return tokenizer.hf_render_chat_template(
        all_messages, add_generation_prompt=True, think=False
    )
...
async def generate_completion(data):
    ...
    def get_token_len(ids):
        if isinstance(ids, torch.Tensor):
            ...
            return (
                ids[0].shape[-1]
                if hasattr(ids[0], "shape") and ids[0].dim() > 0
                else len(ids[0])
            )
    ...
    if tool_calls:
        return {
            ...
            "created": int(time.time()),
            "model": data.get("model", "qwen3.5-9b-exl3"),
            "choices": [
                {
                    "index": 0,
...
async def chat_completions(request: Request):
    ...
    messages = data.get("messages", [])
    tools = data.get("tools", [])
    model_name = data.get("model", "qwen3.5-9b-exl3")
    max_tokens = data.get("max_tokens", MAX_SEQ_LEN - 1024)
