...
示例:
    python3 prompt.py ~/video/1.mp4 ~/video/info.txt
"""
...
def generate_subtitle(
    ...
    if info_file is None:
        info_file = os.path.expanduser("~/video/info.txt")
...
3. 语气：真诚推荐好物分享
4. 不要返回标点符号和英文字母
5. **重要：每段必须12-25个汉字（不含英文和数字）**

示例格式：
百年纯正低频音响界的活化石大品牌|横跨五大领域包括大部分影院还有体育场馆|它真正定义了扩声系统监听扬声器金标准|被哈曼卡顿集团收购的超强美式低音品牌|特别是弹性下潜深度高动态适合流行坚韧耐用|适合喜欢超强重低音的爱好者体验|想要的来我直播间
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("")
        print("示例:")
        print("  python3 prompt.py ~/video/1.mp4 ~/video/info.txt")
        sys.exit(1)
